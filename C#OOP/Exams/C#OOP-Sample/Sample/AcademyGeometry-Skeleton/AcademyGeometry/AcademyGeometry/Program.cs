...
namespace GeometryAPI
{
    ...
    public class Cylinder : Figure, IAreaMeasurable, IVolumeMeasurable
    {
        ...
        {
            return Math.PI * Radius * Radius * (this.top - this.bottom).Magnitude;
        }

        public double GetArea()
        {
            return 2*Math.PI * Radius * Radius+2*(Math.PI*Radius)*((this.top-this.bottom).Magnitude);
        }
                ...
                case "normal":
                    {
                        ...
                        {
                            Console.WriteLine("{0:0.00}", (currentFigure as IFlat).GetNormal().ToString());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
