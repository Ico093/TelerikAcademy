window.onload = function () {
    ...
    //TODO list:
    //implement controls e.g.- pause+resume/clear ranking/custom game
    ...
    function resetGameVariables() {
        ...
        fallingShape = tetromino.getRandomShapeAtPos(SHAPE_INITIAL_X, SHAPE_INITIAL_Y);
        nextShape = tetromino.getRandomShapeAtPos(SHAPE_INITIAL_X, SHAPE_INITIAL_Y);
    ...
    function drawLogo() {
        var paper = Raphael(BOARD_WIDTH * 21.5, 400, 300, 70);
        ...
        paper.path('M 150 10 L 170 10 L L 169 15 L 156 15 L 157 20 H 168 L 166 31 L 160 33 L 154 31 L 153 26 H 158 V 28 L 160 29 L 162 28 L 163 24 H 153 Z').attr({
            stroke: 'none',
            fill: 'white'
        })
    }
        ...
        for (var r = 0; r < currState.length; r++) {
            for (var c = 0; c < currState[r].length; c++) {
                if (currState[r][c] === BLOCK_SYMBOL) {
                    ...
                    var tempY = fallingShape.y + BLOCK_SIZE * r;
                    if (tempY >= 0) {
                        if (tempX + BLOCK_SIZE >= BOARD_WIDTH * BLOCK_SIZE || board[tempY / BLOCK_SIZE][(tempX + BLOCK_SIZE) / BLOCK_SIZE] != undefined) {
                            return true;
                        }
                    }
                }
    ...
    function findMinMaxX() {
        var currState = fallingShape.getCurrentState();
        var minX = 5000; // please correct this with board's dimensions
        var maxX = -5000; // please correct this with board's dimensions
        for (var r = 0; r < currState.length; r++) {
            for (var c = 0; c < currState[r].length; c++) {
                if (currState[r][c] === BLOCK_SYMBOL) {
                    var tempX = fallingShape.x + BLOCK_SIZE * c;
