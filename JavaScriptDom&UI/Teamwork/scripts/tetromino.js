var initializeTetrominoModule = function () {
    ...
    every possible rotation. Only the getRandomShapeAtPos() function is visible to the outside world.
    */
    ...
    var getRandomShapeAtPos = function (x, y) {
        var shape = PIECES[getRandomNumber(0, PIECES.length)];
        var state = getRandomNumber(0, shape.length);
        var color = getRandomColor(50,255);  //avoid too bright colors for the shapes

        return new Shape(shape, state, color, x, y);
    };

    return {
        getRandomShapeAtPos: getRandomShapeAtPos
    }
};